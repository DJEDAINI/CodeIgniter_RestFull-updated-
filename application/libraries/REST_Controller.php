...
abstract class REST_Controller extends CI_Controller {
    ...
    public function __construct($config = 'rest')
    {
            ...
            ($this->config->item('allow_auth_and_keys') === TRUE && $this->_allow === TRUE)))
        {
            ...
            $rest_safe_api = array_map('strtolower', $this->config->item('rest_safe_routes'));
                    // Remove the supported format from the function name e.g. index.json => index
            $object_called = preg_replace('/^(.*)\.(?:'.implode('|', array_keys($this->_supported_formats)).')$/', '$1', $this->router->fetch_method());

            $controller_method = $object_called.'_'.$this->request->method;

            $request_uri = get_class($this).'/'.$controller_method;

            $request_uri = strtolower($request_uri);

            //echo $request_uri;

            if (!in_array( $request_uri,$rest_safe_api)) {
            //print_r($rest_safe_api);
                switch ($rest_auth)
                {
                    case 'basic':
    ...
    protected function _prepare_jwt_auth()
    {
        ...
        if (array_key_exists('Authorization', $headers) && !empty($headers['Authorization'])) {
            ...
            try {
                $decoded = JWT::decode($headers['Authorization'] , JWT::SECRET_KEY, array('HS256'));
                ...
                throw new \Firebase\JWT\ExpiredException($e);

            } catch (\Firebase\JWT\BeforeValidException $e) {
                  
                throw new \Firebase\JWT\BeforeValidException();
