...
this is an update version of the CodeIgniter Rest Server 
Feature : 
...
In this new version i added the jwt authentification, you can enable in by set `$config['rest_auth']` to `'jwt'`.

So to get a response from the server you had to add the authorization token to the header in each request like this `'Authorization : token'`. 

i also add a new endpoint to re-generate the expired token,you can find it in the `'Token'` controller.

you can also add mecanism to add safe routes (No authorization required), you can add them easily in .

```php
$config['rest_safe_routes'] = ['ControlerName/MethodName_RequestMethod', .... ];
```
